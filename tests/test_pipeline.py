...
def test_the_opener_discloses_and_says_nothing_else(_isolated_db):
    """Apple wants the disclosure in the audio itself, not only the metadata --
    and a listener needs it in the first few seconds. Naming the works here is
    what made the opener long enough to lose people, so it does not."""
    import db
    ...
    text = intro.opener_text(db.get_episode("EINTRO"), _intro_cfg())
    assert "AI generated" in text
    assert "Minimum Wages and Employment" not in text
    assert len(text.split()) < 12, "anything longer belongs in the credits"


def test_the_credits_name_the_work_and_disclose_again(_isolated_db):
    import db
    from pipeline import intro

    db.create_episode("EINTRO1B", "/tmp/i.pdf", "sha-intro1b")
    db.update_principal("EINTRO1B", title="Minimum Wages and Employment", authors=json.dumps(["David Card", "Alan Krueger"]))

    text = intro.credits_text(db.get_episode("EINTRO1B"), _intro_cfg())
    assert "generated by AI" in text
...
def test_intro_is_off_when_switched_off(_isolated_db):
    ...
    row = db.get_episode("EINTRO2")
    assert intro.opener_text(row, _intro_cfg(enabled=False)) is None
    assert intro.credits_text(row, _intro_cfg(enabled=False)) is None


def test_turning_the_opener_off_leaves_only_the_credits(_isolated_db):
    """The honest version of "put it all at the end": still disclosed, but a
    listener who stops early never hears it."""
    import db
    from pipeline import intro

    db.create_episode("EINTRO2B", "/tmp/i.pdf", "sha-intro2b")
    db.update_principal("EINTRO2B", title="A Paper", authors=json.dumps(["Solo"]))
    cfg = _intro_cfg(opener=False)
    row = db.get_episode("EINTRO2B")

    assert intro.opener_text(row, cfg) is None
    assert "generated by AI" in intro.credits_text(row, cfg)


def test_credits_do_not_announce_an_uncredited_author(_isolated_db):
    ...
    text = intro.credits_text(db.get_episode("EINTRO3"), _intro_cfg())
    assert "uncredited" not in text
    assert text.endswith("A Paper With No Byline.")


def test_credits_do_not_double_the_full_stop_after_et_al(_isolated_db):
    import db
    from pipeline import intro
    ...
    text = intro.credits_text(db.get_episode("EINTRO4"), _intro_cfg())
...
def test_intro_uses_one_voice_that_is_neither_host(_isolated_db, monkeypatch):
    ...
    assert "generated by AI" in captured["contents"], "the last call is the credits"
    assert intro.recorded_text("EINTRO5", intro.OPENER).startswith("This is Paperpod")
    assert intro.wav_path("EINTRO5", intro.OPENER).exists()
    assert intro.wav_path("EINTRO5", intro.CREDITS).exists()
...
def test_intro_is_resynthesized_only_when_its_words_change(_isolated_db, monkeypatch):
    ...
    assert len(calls) == 2, "an opener and a set of credits"
    intro.synthesize_intro("EINTRO6", _intro_cfg())
    assert len(calls) == 2, "unchanged wording must reuse the WAVs on disk"

    db.update_principal("EINTRO6", title="Second Title")
    intro.synthesize_intro("EINTRO6", _intro_cfg())
    assert len(calls) == 3, "only the credits name the work, so only they change"
    assert "Second Title" in calls[2]
    assert intro.recorded_text("EINTRO6", intro.CREDITS) == calls[2]
...
def test_shipped_intro_template_reads_as_a_sentence(_isolated_db):
    ...
    cfg = load_config()
    row = db.get_episode("EINTRO7")
    opener = intro.opener_text(row, cfg)
    credits = intro.credits_text(row, cfg)
    assert opener.startswith("This is Paperpod, an AI generated podcast")
    assert credits.endswith("It drew on Some Important Paper, by Ada Lovelace.")
    assert "$" not in opener + credits, "every placeholder must have been substituted"
...
def test_the_credits_read_out_every_work_including_the_references(_isolated_db):
    """If a work was worth drawing on it is worth naming. Counting them was
    the cheaper option and it turned the episode's own basis into a
    footnote."""
    import db
    from pipeline import intro
    ...
    text = intro.credits_text(db.get_episode("EMULTIINTRO"), cfg)
    assert "Minimum Wages and Employment, by David Card and Alan Krueger" in text
    assert "Employment Effects Reconsidered, by David Neumark" in text
    assert "; and A Third Thing, by Someone" in text, (
        "the reference is read out too, and last")
...
def test_one_paper_still_gets_the_single_work_disclosure(_isolated_db):
    ...
    text = intro.credits_text(db.get_episode("ESOLOINTRO"), {"intro": {"enabled": True}})
    assert text.endswith("It drew on A Paper, by Solo Author.")
